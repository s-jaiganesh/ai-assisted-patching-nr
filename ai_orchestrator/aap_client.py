...
class AAPClient:
    ...
    # ------------------------------------------------------------------
    # NEW METHOD (inventory snapshot helper)
    # ------------------------------------------------------------------
    def get_hosts_in_group(self, inventory_id: int, group_name: str) -> list[str]:
        """
        Wrapper helper used by orchestrator to freeze host list before launching AAP job.
        Internally reuses existing get_hosts_by_group_name logic.
        """
        return self.get_hosts_by_group_name(inventory_id, group_name)
    
    def get_project_update(self, pu_id: int) -> dict:
        url = f"{self.base_url}/api/v2/project_updates/{pu_id}/"
        r = self.s.get(url, verify=self.verify_ssl, timeout=60)
        r.raise_for_status()
        return r.json()