...
def validate_email_attachments(attachments: Dict[str, str], required_keys: List[str]) -> List[str]:
    missing: List[str] = []
...
def trigger_email_job(
    ...
    job_template_id = email_cfg.get("job_template_id")
    if not job_template_id:
        raise ValueError("communications.email.job_template_id missing")
    ...
    required_keys = ["patch_report", "apm_report"]
    missing = validate_email_attachments(attachments, required_keys)
    if missing:
        raise FileNotFoundError(f"Email attachment validation failed for: {', '.join(missing)}")
    ...
    extra_vars = {
        ...
        "patch_report_path": attachments["patch_report"],
        "apm_report_path": attachments["apm_report"],
        "total_servers": int(counts.get("TOTAL", 0)),
        "success_count": int(counts.get("SUCCESS", 0)),
        "failed_count": int(counts.get("FAILED", 0)),
        "operations_note": email_cfg.get(
            "operations_note",
            "Please review the attached patch and APM reports and follow standard incident handling for any failed servers.",
        ),
    }

    return client.launch_job_template(int(job_template_id), limit="localhost", extra_vars=extra_vars)
...
def main():
    ...
    try:
        email_job_id = trigger_email_job(client, email_cfg, meta["change_id"], summary, attachments)
        if email_job_id:
            safe_post(use_teams, webhook, f"{meta['change_id']} | email report job launched | job_id={email_job_id}")
    except Exception as exc:
