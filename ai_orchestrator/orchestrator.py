...
from .decision_gate import ai_remediation_plan
...
def fetch_monitoring(nr_client: Optional[NewRelicClient], pg_cfg: dict, table_name: str, hosts: List[str], traffic_minutes: int) -> None:
    if not nr_client:
        return
    for host in hosts:
        try:
...
def main():
    ...
    client = AAPClient(aap_cfg["controller_url"], aap_token, verify_ssl=bool(aap_cfg.get("verify_ssl", True)))
    nr_client = None
    if nr_enabled:
    ...
    from .decision_gate import ai_select_waves
    ...
    selected_groups = sorted(selected_groups, key=lambda g: parse_group_name_for_schedule(g, cw_start, tz) or datetime.max.replace(tzinfo=tz))

    safe_post(use_teams, webhook, fmt_waiting_message(meta["change_id"], {g: waiting_by_wave[g] for g in selected_groups if g in waiting_by_wave}))
    ...
    for inv_group in selected_groups:
        ...
        for h in wave_hosts.get(inv_group, []):
            if h in host_first_wave:
                record_duplicate_host(pg_cfg, table_name, original_host=h, dup_server_key=f"{h}_dup_{inv_group}", current_wave=inv_group, patched_wave=host_first_wave[h])
                continue
        ...
        for inv_group in selected_groups:
            ...
            if scheduled_dt and now >= scheduled_dt:
                ...
                if not hosts:
                    ...
                    continue
                safe_post(use_teams, webhook, f"{meta['change_id']} | {inv_group} | launching parent wave for {len(hosts)} hosts")
                pipeline = launch_patch_pipeline(
                    client, patch_job_template_id, meta, pg_cfg, table_name, inv_group, hosts, "pre_check", False, f"{inv_group}:main"
                )
                pipelines[pipeline.pipeline_id] = pipeline
                launched_waves.add(inv_group)

        # process remediation jobs first
        for remediation_id, rem in list(remediation_jobs.items()):
            ...
            if is_terminal_status(status):
                ...
                if status == "successful":
                    rerun_hosts = sorted(rem.fix_plan.keys())
                    safe_post(use_teams, webhook, f"{meta['change_id']} | {rem.wave_name} | remediation successful for stage {rem.stage} on {', '.join(rerun_hosts)} | launching rerun")
                    ...
                    for host, fix_type in rem.fix_plan.items():
                        failed_terminal[host] = {"stage": rem.stage, "reason": f"remediation_failed:{fix_type}; {job.get('job_explanation') or status}"}
                    safe_post(use_teams, webhook, f"{meta['change_id']} | {rem.wave_name} | remediation failed for stage {rem.stage} | hosts={', '.join(sorted(rem.fix_plan.keys()))}")
                rem.status = "done"
            else:
        ...
        for pipeline_id, pipe in list(pipelines.items()):
            ...
            if completion["completed"]:
                ...
                if failed_hosts:
                    ...
                    actionable = {h: f for h, f in fix_plan.items() if f in {"boot_cleanup", "rpm_db_rebuild", "kernel_reinstall"}}
                    for host, fix_type in fix_plan.items():
                        if fix_type not in {"boot_cleanup", "rpm_db_rebuild", "kernel_reinstall"}:
                            failed_terminal[host] = {"stage": stage, "reason": results["failed_reasons"].get(host, f"no_safe_fix:{fix_type}")}
                    if actionable:
                        safe_post(use_teams, webhook, f"{meta['change_id']} | {pipe.wave_name} | remediation stage {stage} | fixes={actionable}")
                        rem_job_id = launch_remediation_job(
                            client, remediation_job_template_id, meta, pg_cfg, table_name, pipe.wave_name, stage, actionable
                        )
            ...
            # stage not complete yet; check AAP health + timeout
            ...
            if status in {"failed", "error", "canceled"} and completion["pending_hosts"]:
                ...
                pipe.status = "failed_terminal"
                safe_post(use_teams, webhook, f"{meta['change_id']} | {pipe.wave_name} | pipeline {pipe.pipeline_id} terminal AAP status {status} on stage {stage}")
    ...
    safe_post(
        use_teams,
        webhook,
        f"{meta['change_id']} | completed | success={summary['counts'].get('SUCCESS',0)} failed={summary['counts'].get('FAILED',0)} warning={summary['counts'].get('WARNING',0)} degraded={summary['counts'].get('DEGRADED',0)} | reports={attachments['patch_report']}, {attachments['apm_report']}",
    )
    try:
