...
def ensure_tables(conn, table: str):
    """
    Ensures both the main host status table and the new APM metrics table exist.
    """
    with conn.cursor() as cur:
        # Main table for host-level status (Ansible + Infra)
        cur.execute(f"""
        ...
        # New, separate table for APM service metrics (one-to-many)
        cur.execute(f"""
...
def seed_hosts(pg: dict, table: str, hosts: List[str]):
    ...
    with connect(pg) as conn:
        ensure_tables(conn, table)
        with conn.cursor() as cur:
            for h in hosts:
                cur.execute(f"INSERT INTO {table}(server) VALUES (%s) ON CONFLICT (server) DO NOTHING", (h,))
        conn.commit()
...
def update_monitoring(pg: dict, table: str, hostname: str, phase: str, snap: Dict[str, Any]):
    with connect(pg) as conn:
        ...
        apm_table = f"{table}_apm"
        with conn.cursor() as cur:
            # Always ensure the host row exists in the main table
            cur.execute(f"INSERT INTO {table}(server) VALUES (%s) ON CONFLICT (server) DO NOTHING", (hostname,))

            # Case 1: Host not found in New Relic at all.
            ...
            # --- If we get here, the host WAS found. ---

            # Update host-level infra data in the main table
            cur.execute(
                f"""
                UPDATE {table}
                SET nr_host_guid=%s,
            ...
            # Now, update the APM services in the separate APM table
            apm_services = snap.get("apm_services", [])
            for service in apm_services:
                if phase == "pre":
                    # For the 'pre' phase, we INSERT the service row if it doesn't exist, and update if it does.
                    cur.execute(
                        f"""
                    ...
                    # For the 'post' phase, we just update the existing row with the post-patching data.
                    cur.execute(
                        f"""
                        UPDATE {apm_table}
