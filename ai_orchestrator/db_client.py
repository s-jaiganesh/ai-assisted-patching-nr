...
STAGES = ["pre_check", "apply_patch", "post_reboot", "post_check", "kernel_check"]
TERMINAL_STAGE_VALUES = {"success", "failed", "skipped"}
SUCCESS_STAGE_VALUES = {"success", "skipped"}
...
def fetch_all_rows(pg: dict, table: str) -> List[Dict[str, Any]]:
    with connect(pg) as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(f"SELECT * FROM {table} ORDER BY wave_name NULLS LAST, server")
            return [dict(r) for r in cur.fetchall()]
...
def fetch_all_apm_rows(pg: dict, table: str) -> List[Dict[str, Any]]:
    apm_table = f"{table}_apm"
    with connect(pg) as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(f"SELECT * FROM {apm_table} ORDER BY server, nr_apm_name")
            return [dict(r) for r in cur.fetchall()]


def get_stage_rows(pg: dict, table: str, hosts: List[str], stage: str) -> List[Dict[str, Any]]:
    if not hosts:
        return []
    reason_col = f"{stage}_reason"
    with connect(pg) as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                f"SELECT server, wave_name, reachable, {stage} AS stage_value, {reason_col} AS stage_reason, updated_at "
                f"FROM {table} WHERE server = ANY(%s) ORDER BY server",
                (hosts,),
            )
            return [dict(r) for r in cur.fetchall()]


def get_stage_completion(pg: dict, table: str, hosts: List[str], stage: str) -> Dict[str, Any]:
    rows = get_stage_rows(pg, table, hosts, stage)
    by_server = {r["server"]: r for r in rows}
    completed = []
    pending = []
    for host in hosts:
        row = by_server.get(host)
        value = str((row or {}).get("stage_value") or "").strip().lower()
        reason = (row or {}).get("stage_reason")
...
def split_stage_results(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for row in rows:
        host = row.get("server")
        value = str(row.get("stage_value") or "").strip().lower()
        reason = str(row.get("stage_reason") or "").strip()
        if value in SUCCESS_STAGE_VALUES:
            success_hosts.append(host)
        elif value == "failed":
            failed_hosts.append(host)
            failed_reasons[host] = reason
        else:
            pending_hosts.append(host)

    return {
        "success_hosts": sorted(set(success_hosts)),
        "failed_hosts": sorted(set(failed_hosts)),
        "pending_hosts": sorted(set(pending_hosts)),
        ...
        with conn.cursor() as cur:
            ...
            for service in apm_services:
                if phase == "pre":
                    ...
                    cur.execute(
                        f"""
                        INSERT INTO {apm_table} (server, nr_apm_guid, nr_apm_name, nr_apm_account_id, apm_post_alert, apm_post_traffic, updated_at)
                        VALUES (%s, %s, %s, %s, %s, %s, now())
                        ON CONFLICT (server, nr_apm_guid)
                        DO UPDATE SET
                          nr_apm_name = EXCLUDED.nr_apm_name,
                          nr_apm_account_id = EXCLUDED.nr_apm_account_id,
                          apm_post_alert = EXCLUDED.apm_post_alert,
                          apm_post_traffic = EXCLUDED.apm_post_traffic,
                          updated_at = now()
                        """,
                        (hostname, service.get("guid"), service.get("name"), service.get("account_id"), service.get("alert"), service.get("traffic"))
                    )
